...
class Model_Content {
	...
	public static function parseContentBodyItems($original_content, $title) {
						...
						if (stristr ( $image, $media_url ) == true) {
							...
							if ($this->core_model->url_IsFile ( $image ) == true) {
								...
								if (stristr ( $image, $media_url ) == false) {
									...
									// $this->core_model->url_getPageToFile (
	...
	function contentGetPagesWithCategories() {
		...
		// $taxonomies = $this->core_model->dbQuery ( $q, $cache_id = md5 ( $q
		// ), $cache_group = 'content' );
		

		$pages = $this->core_model->dbQuery ( $q, $cache_id = md5 ( $q ), $cache_group = 'content/global' );
	...
	function getParentPagesIdsForPageId($id) {
		...
		$taxonomies = $this->core_model->dbQuery ( $q, $cache_id = md5 ( $q ), $cache_group = 'content/' . $id );
	...
	function contentGetHrefForPostId($id) {
					...
					if ($url == false) {
						...
						foreach ( $master as $m ) {
							...
							if ($m ['id'] == $c ['id']) {
								...
								$url = $this->taxonomy_model->getUrlForIdAndCache ( $m ['id'] ); // .
								// '/'
								// .
								// $content
								// ['content_url'];
		...
		/*
			 ...
			 * where cache_id='$cache_id' "; $this->core_model->dbQ ( $q );
			 * $this->core_model->cacheDeleteFile ( $cache_id );
			 * $this->core_model->cacheWriteContent ( $cache_id, $cache ); $q =
			 * "INSERT INTO $table_cache set cache_id='$cache_id',
			 * cache_group='content' "; $this->core_model->dbQ ( $q );
			 */
		
		// cache
		}
		
		$cache = $this->core_model->cacheWriteAndEncode ( $the_url, $function_cache_id, $cache_group );
	...
	function contentGetThumbnailForContentId($id, $size = 128, $size_h = false) {
		...
		$i = $this->core_model->extractTags ( $html, 'img', $selfclosing = true, $return_the_entire_tag = false, $charset = 'UTF-8' );
		if (! empty ( $i [0] )) {
		...
		if ($src != '') {
			...
			$this->core_model->cacheWriteAndEncode ( $src, $function_cache_id, $cache_group );
	...
	function contentGenerateUniqueUrlTitleFromContentTitle($the_id, $the_content_title, $the_content_url_overide = false) {
		...
		if (intval ( $the_id ) != 0) {
			...
			$the_new_content_url = $this->core_model->url_title ( $the_content_title, 'dash', true );
	...
	function contentActiveCategoriesForPageId2($page_id, $url, $no_base = false) {
					...
					if ($item != '') {
						...
						if (! empty ( $item )) {
							...
							if (strtolower ( $item [0] ) == 'category') {
								...
								// $possible_ids =
								// $this->taxonomy_model->taxonomyGetTaxonomyIdsForTaxonomyRootIdAndCache
							...
							if ($categories_get_from_url != false) {
								...
								if (! empty ( $taxonomy_values )) {
									...
									foreach ( $taxonomy_values as $taxonomy_value ) {
										 ...
										 * $this->taxonomy_model->taxonomyGet (
										 * $data );
										 */
										...
										if (! empty ( $results )) {
											...
											foreach ( $results as $res ) {
												...
												if (intval ( $res ["id"] ) != 0) {
													...
													$pars1 = $this->taxonomy_model->getParentsIdsAndCache ( $res ["id"], true );
													// .p($pars1,1);
													if (! empty ( $pars1 )) {
		...
		/*
			 ...
			 * $this->core_model->cacheWriteAndEncode ( $categories_to_return,
			 * $function_cache_id ); return $categories_to_return; } else {
			 * $this->core_model->cacheWriteAndEncode ( 'false',
	...
	function contentGetByParams($params) {
		...
		$is_search = $this->core_model->getParamFromURL ( 'search' );
		// p($is_search,1);
		if ($is_search != false) {
			$segs = url ();
			$segs = explode ( '/', $segs );
	...
	function getContent($data, $orderby = false, $limit = false, $count_only = false, $short_data = false, $only_fields = false) {
		...
		global $cms_db_tables;
		$use_fetch_db_data = false;
		 
		if (!empty($data) and array_key_exists('use_fetch_db_data', $data)) {
	...
	$this->load->model ( 'Taxonomy_model', 'taxonomy_model' );
				...
				if (is_array ( $categories ) and ! empty ( $categories )) {
					...
					foreach ( $categories as $item ) {
						...
						if (intval ( $item ) != 0) {
							...
							$item = $this->taxonomy_model->getIdByName ( $item );
							// p($item);
							if (intval ( $item ) != 0) {
								$category_ids [] = $item;
								$more = $this->taxonomy_model->getChildrensRecursiveAndCache ( $item, 'category' );
								// p($more);
								if (! empty ( $more )) {
									foreach ( $more as $mor ) {
										// p($mor);
					...
					$only_thise_content = $this->taxonomy_model->getToTableIds ( end ( $category_ids ), false );
		...
		if (trim ( strval ( $data ['search_by_keyword'] ) ) != '') {
			...
			$seach_in_cats = $this->taxonomy_model->getIdsByNames ( $categories );
			//
			

			$seach_in_cats = $this->taxonomy_model->getToTableIds ( $seach_in_cats, false );
			// p($seach_in_cats);
			...
			// $keyword_results = $this->core_model->dbRegexpSearch ( $table,
			// $kw, $kw_ids );
...
select to_table_id from $table_comments where to_table = 'table_content' group by to_table_id )
		...
		// $this->core_model->fetchDbData thats why we romove it from here
...
$my_limit_q
			...
			$q = $this->core_model->dbQuery ( $q, md5 ( $q ), 'votes/global' );
...
$my_limit_q
		...
		if ($count_only == true) {
			...
			// $this->core_model->cacheWriteAndEncode ( $save,
	...
	public static function applyGlobalTemplateReplaceables($content, $replaceables = false) {
		static $mem = array ();
	...
	function saveMenuItem($data) {
		...
		if (intval ( $data ['id'] ) != 0) {
			...
			$check = $this->core_model->dbQuery ( $check_if_exists );
			
			$check = $check [0] ['qty'];
			// p($check);
			if (intval ( $check ) == 0) {
				$check_if_exists = " INSERT into $table set id={$data ['id']}";
				$insert = $this->core_model->dbQ ( $check_if_exists );
			}
		}
	...
	function getBreadcrumbsByURLAndPrintThem($the_url = false, $include_home = true) {
		...
		if (! empty ( $quick_nav )) {
			...
			foreach ( $quick_nav as $item ) {
				...
				?><li><a href="<?php print $item['url'];?>"><?php
...
// $active_categories = $this->taxonomy_model->getCategoriesForContent (
...
$this->core_model->cacheWriteAndEncode ( $id, $function_cache_id, $cache_group = 'menus' );
...
$active_class_titl = $this->core_model->url_title ( strtolower ( $thecontent_title ['content_title'] ) );
...
// print "[[ $time ]]seconds\n";
$to_print .= '</ul>';
$this->core_model->cacheWriteAndEncode ( $to_print, $function_cache_id, 'menus' );
return $to_print;
}
...
// $data = $this->taxonomy_model->taxonomyGenerateTagsFromString (
// $data, 3 );
...
// $keyrods = $this->taxonomy_model->taxonomyGenerateTagsFromString
...
//
$q = $this->core_model->dbQuery ( $sql, $cache_id = 'content_helpers_getCaregoriesUlTree_parent_cats_q_' . md5 ( $sql ), 'taxonomy/' . intval ( $content_parent ) );
...
* to_table_id ;"; //$q = $this->core_model->dbQuery
...
* $this->core_model->dbQuery ( $q_1, $cache_id =
...
// $childern_content = $this->taxonomy_model->getItems (
...
$server = array ('Google' => 'http://blogsearch.google.com/ping/RPC2', 'Feed Burner' => 'http://ping.feedburner.com/', 'Moreover' => 'http://api.moreover.com/RPC2', 'Syndic8' => 'http://ping.syndic8.com/xmlrpc.php', 'BlogRolling' => 'http://rpc.blogrolling.com/pinger/'
, 'Technorati' => 'http://rpc.technorati.com/rpc/ping', 'Ping-o-Matic!' => 'http://rpc.pingomatic.com/' );
...
// $media = $this->core_model->mediaGet ( $to_table =
