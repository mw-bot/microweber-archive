<?
//$mem = cache_handle_on_load(true);
//d($mem);
//document_ready('cache_handle_on_load');
function cache_handle_on_load($cache_id = false) {
	$url = url_string();
	$function_cache_id = __FUNCTION__ . crc32($url);
	...
	if (is_bool($cache_id) and $cache_id == true) {
		...
		$cf = cache_get_content_from_memory('all');
		//	$cf = base64_encode($cf);
		//$cf = serialize($cf);
		cache_store_data($cf, $function_cache_id, $cache_group);
		return $cf;
	}
...
function cache_get_content_from_memory($cache_id, $cache_group = false, $replace_with_new = false) {
	...
	if (is_string($cache_id) and $cache_id == "all") {
		return $mem;
	}
