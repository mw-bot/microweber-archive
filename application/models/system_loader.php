...
# Timestamps may not be unique, file names are.
								}
						}
						else
								if ($dirs_or_files == 'dirs') {
...
function str2num($sNumber) {
		...
		$bIsNegative = (0 === $aConventions['n_sign_posn'] && '(' === $sNumber {
				0
		}
		&& ')' === $sNumber {
				strlen($sNumber) - 1
		...
		if (strlen($sNegativeSign) && 0 !== $aConventions['n_sign_posn']) {
				$bIsNegative = ($sNegativeSign === $sNumber {
						0
				}
				|| $sNegativeSign === $sNumber {
						$iLength - 1
				}
				);
				if ($bIsNegative) {
...
// how many of thse child nodes do we have?
						if ($iChildCount == 1) {
// only one child ï¿½ make associative array
								$arResult[$sKey] = $mValue;
						}
						elseif ($iChildCount > 1) {
// more than one child like this ï¿½ make numeric array
								$arResult[$sKey][] = $mValue;
						}
						elseif ($iChildCount == 0) {
...
// anchor
				...
				$urlenc = substr($url, 0, $firstsep);
// encoded origin uri
				$urlenc .= "?" . implode($phpsep, $valsenc) .
...
//or modify this call if it doesn't fit your situation.
				...
				$tag = array('tag_name' => $match['tag'][0], 'match' => $match, 'offset' => $match[0][1], 'contents' => !empty ($match['contents']) ? $match['contents'][0] : '',
...
//there is a lower- and upperChar, / Char is case sentitive
						$return .= '[' . $myChar . $myUpperChar . ']';
//adding both Versions : [xX]
				}
				else {
//only one case Version / Char is case insentitive
						$return .= $myChar;
//adding '1','.','*',...
				}
		}
