...
,  updated_on='{$updated_on}'
	...
	function save_field_old() {
		...
		/*	if (! empty ( $_POST )) {
		 ...
		 $html = $this->template_model->parseMicrwoberTags ( $v1_v, $options = false );
		 $v2 [$v1_k] =  ( $html );
		 }
	...
	function temp2() {
		...
		foreach ($q as $post) {
			...
			$save = $this -> core_model -> saveData($table_content, $to_save);
			...
			foreach ($html->find ( 'div.detail-wrap' ) as $article) {
				...
				foreach ($c as $c1) {
					...
					if ($c_data['taxonomy_value'] != '') {
						$add_new = $this -> taxonomy_model -> taxonomySave($c_data, $preserve_cache = true);
						$tax_cats[] = $add_new;
					}
				...
				if (stristr($item['custom_field_pdf'], '.pdf')) {
					...
					$this -> core_model -> url_getPageToFile($item['custom_field_pdf'], $f1234zf);
				...
				$is_p = $this -> content_model -> getContentByURLAndCache($itit);
				//. p($is_p);
				//  $is_post = $this->content_model->getContentAndCache ( $is_post, $orderby1, array (0, 10 ), $short_data = false, $only_fields = array ('id', 'content_title', 'content_body', 'content_url', 'content_filename', 'content_parent', 'content_filename_sync_with_editor', 'content_body_filename' ) );
				...
				$to_save = $this -> content_model -> saveContent($item, $delete_the_cache = false);
